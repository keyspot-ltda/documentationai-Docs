...
<CodeGroup tabs="Bash,TypeScript,Python">
  ...
  curl -X POST https://partner-api.keyspot.app/v1/auth/token \
    -H "X-API-Key: ks_partner_abc123def456" \
    -H "X-API-Secret: ks_secret_xyz789"
  ```
  ...
  const token = await getToken("ks_partner_abc123def456", "ks_secret_xyz789");
  ```
  ...
  token = get_token("ks_partner_abc123def456", "ks_secret_xyz789")
  ```
</CodeGroup>
...
```bash
curl https://partner-api.keyspot.app/v1/properties?fields=code,title \
...
- Se suas credenciais forem comprometidas, solicite novas à equipe Keyspot imediatamente

### Restrição por IP

A Keyspot pode configurar uma lista de IPs permitidos para suas credenciais. Se o IP da requisição não estiver na lista, a API retorna `403 IP_BLOCKED`. Solicite a inclusão de novos IPs à equipe Keyspot.
